"use client"; // <-- NEW: This makes it a Client Component
...
export function ModernFooter() {
  // <-- UPDATED: Changed name and added 'export'
  const toolLinks = [
  ...
  const companyLinks = [
    ...
    {
      name: "support@smoothledger.com",
      href: "mailto:support@smoothledger.com",
    },
  ];
  ...
  // NEW: This handler is now safe inside a Client Component
  const handleNewsletterSubmit = (e) => {
    e.preventDefault();
    // This is where you would wire up an API route
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className="lg:col-span-4">
            ...
            >
              Smooth
              <span className="text-blue-600 dark:text-blue-500">Ledger</span>
            </Link>
            <p className="mt-2 text-base text-slate-500 dark:text-slate-400">
              Financial Tools, Beautifully Simple.
            </p>
            <div className="mt-6 flex space-x-4">
              <a href="https://www.crunchbase.com/organization/smoothledger" className="text-slate-400 hover:text-slate-500">
                <SiCrunchbase className="h-6 w-6" />
              </a>
              <a href="https://github.com/Mutashim99" className="text-slate-400 hover:text-slate-500">
                <RiGithubFill className="h-6 w-6" />
              </a>
              <a href="https://www.linkedin.com/company/smoothledger" className="text-slate-400 hover:text-slate-500">
          ...
          <div className="lg:col-span-8 grid grid-cols-2 md:grid-cols-4 gap-8">
              ...
              <ul className="mt-4 space-y-2">
                {toolLinks.map((link) => (
                  <li key={link.name}>
                    <Link
                      href={link.href}
                      className="text-base text-slate-600 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white"
                    >
                      {link.name}
                    </Link>
                  </li>
              ...
              <ul className="mt-4 space-y-2">
                {companyLinks.map((link) => (
                  <li key={link.name}>
                    <Link
                      href={link.href}
                      className="text-base text-slate-600 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white block break-all"
                    >
                      {link.name}
                    </Link>
                  </li>
            ...
            <div className="md:col-span-2">
              ...
              <p className="mt-4 text-base text-slate-600 dark:text-slate-300">
                Get the latest news, articles, and free tools, sent straight to
                your inbox.
              </p>
              {/* UPDATED: This form is now safely in a Client Component */}
              <form
              ...
              >
                ...
                >
                  <RiSendPlaneFill className="h-5 w-5" /> Send
                </button>
              </form>
            </div>
          </div>
